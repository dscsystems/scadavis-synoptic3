...
/**
 * sprintf implementation - JavaScript emulation of the C printf function
 * @param fstring - Format string
 * @param args - Values to format
 * @returns Formatted string
 */
export function sprintf (fstring: string, ...args: any[]): string {
  const pad = function (str: string, ch: string, len: number): string {
    let ps = ''
    for (let i = 0; i < Math.abs(len); i++) ps += ch
    return len > 0 ? str + ps : ps + str
  }

  const processFlags = function (
    flags: string,
    width: string,
    rs: string,
    arg: number
  ): string {
    const pn = function (flags: string, arg: number, rs: string): string {
      if (arg >= 0) {
  ...
  const converters: Record<
    string,
    (flags: string, width: string, precision: string, arg: any) => string
  > = {}
  converters['c'] = function (_flags, _width, _precision, arg) {
  ...
  converters['i'] = function (flags, width, precision, arg) {
    const iPrecision = parseInt(precision)
  ...
  converters['e'] = function (flags, width, precision, arg) {
    const iPrecision = parseInt(precision)
    const prec = isNaN(iPrecision) ? 6 : iPrecision
    let rs = Math.abs(arg).toExponential(prec)
  ...
  converters['f'] = function (flags, width, precision, arg) {
    const iPrecision = parseInt(precision)
    const prec = isNaN(iPrecision) ? 6 : iPrecision
    let rs = Math.abs(arg).toFixed(prec)
  ...
  let retstr = farr[0]
  const fpRE = /^([-+ #]*)(\d*)\.?(\d*)([cdieEfFgGosuxX])(.*)$/
  for (let i = 1; i < farr.length; i++) {
    const fps = fpRE.exec(farr[i])
    if (!fps) continue
    if (arguments[i] != null)
      ...
      if (
        ...
        switch (state.InkSage[i].attr) {
          ...
          case 'get':
            ...
            // Handle NaN case - if val is not a valid number, use it as-is (string)
            const numVal = parseFloat(val)
            const displayVal = isNaN(numVal) ? val : String(numVal)
            if (
              state.InkSage[i].parent &&
              displayVal !==
              ...
              if (state.InkSage[i].parent?.firstElementChild?.nodeName === "tspan") {
                ...
                state.InkSage[i].parent.textContent = displayVal
              }
